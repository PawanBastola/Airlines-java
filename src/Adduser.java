...
//account_register panel ends here
        ...
        adduser.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                ...
                // ends
                try {
                    String sqlemp = "INSERT INTO `employee`(first_name,last_name,gender,phone,email,date_of_birth,address) VALUES('" + fname + "','" + lname + "','"
                            + sgender + "','" + sphone + "','" + semail + "','" + sdob + "','" + saddress + "')";

                    connect.insert(sqlemp);

                    String sqlquery = "SELECT max(empid) from `employee`";
                    ResultSet rs = connect.display(sqlquery);
                    while (rs.next()) {
                        int max = rs.getInt(1);

                        String sql = "INSERT INTO `adminanduser` VALUES('" + max + "','" + uname + "','" + confirmedpass + "','" + utype + "')";
